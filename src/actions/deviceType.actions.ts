"use server";

import { revalidatePath } from "next/cache";
import { z } from "zod";
import { getTranslations } from "next-intl/server";
import { prisma } from "@/lib/prisma";
import { auth } from "@/lib/auth";

export type DeviceTypeActionState = {
  success: boolean;
  message: string;
  errors?: Record<string, string[]>;
};

async function buildDeviceTypeSchema() {
  const t = await getTranslations("DeviceTypeActions");
  return z.object({
    name:     z.string().min(1, t("nameRequired")),
    isActive: z.boolean(),
  });
}

function readDeviceTypeForm(formData: FormData) {
  return {
    name:     ((formData.get("name") as string) ?? "").trim(),
    isActive: formData.get("isActive") !== "false",
  };
}

export async function createDeviceTypeAction(
  _prev: DeviceTypeActionState,
  formData: FormData
): Promise<DeviceTypeActionState> {
  const session = await auth();
  const t = await getTranslations("DeviceTypeActions");
  if (!session?.user) return { success: false, message: t("loginRequired") };

  const schema = await buildDeviceTypeSchema();
  const parsed = schema.safeParse(readDeviceTypeForm(formData));
  if (!parsed.success) {
    return { success: false, message: t("invalidData"), errors: parsed.error.flatten().fieldErrors };
  }

  try {
    await prisma.deviceType.create({
      data: { name: parsed.data.name, isActive: parsed.data.isActive },
    });
    revalidatePath("/repairs/device-types");
    return { success: true, message: t("createSuccess") };
  } catch (e: any) {
    if (e.code === "P2002") return { success: false, message: t("duplicateName") };
    return { success: false, message: t("genericError") };
  }
}

export async function updateDeviceTypeAction(
  _prev: DeviceTypeActionState,
  formData: FormData
): Promise<DeviceTypeActionState> {
  const session = await auth();
  const t = await getTranslations("DeviceTypeActions");
  if (!session?.user) return { success: false, message: t("loginRequired") };

  const id = Number(formData.get("id"));
  if (!id) return { success: false, message: t("notFound") };

  const schema = await buildDeviceTypeSchema();
  const parsed = schema.safeParse(readDeviceTypeForm(formData));
  if (!parsed.success) {
    return { success: false, message: t("invalidData"), errors: parsed.error.flatten().fieldErrors };
  }

  try {
    await prisma.deviceType.update({
      where: { id },
      data: { name: parsed.data.name, isActive: parsed.data.isActive },
    });
    revalidatePath("/repairs/device-types");
    return { success: true, message: t("updateSuccess") };
  } catch (e: any) {
    if (e.code === "P2002") return { success: false, message: t("duplicateName") };
    if (e.code === "P2025") return { success: false, message: t("notFound") };
    return { success: false, message: t("genericError") };
  }
}

export async function deleteDeviceTypeAction(id: number): Promise<DeviceTypeActionState> {
  const session = await auth();
  const t = await getTranslations("DeviceTypeActions");
  if (!session?.user) return { success: false, message: t("loginRequired") };

  const inUse = await prisma.repairJob.count({ where: { deviceTypeId: id } });
  if (inUse > 0) return { success: false, message: t("inUse", { count: inUse }) };

  try {
    await prisma.deviceType.delete({ where: { id } });
    revalidatePath("/repairs/device-types");
    return { success: true, message: t("deleteSuccess") };
  } catch (e: any) {
    if (e.code === "P2025") return { success: false, message: t("notFound") };
    return { success: false, message: t("genericError") };
  }
}

export async function getDeviceTypes(includeInactive = true) {
  return prisma.deviceType.findMany({
    where: includeInactive ? {} : { isActive: true },
    orderBy: [{ isActive: "desc" }, { name: "asc" }],
  });
}
