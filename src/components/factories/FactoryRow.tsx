...
import {
  updateFactoryAction,
  deleteFactoryAction,
  type FactoryActionState,
} from "@/actions/factory.actions";

const initState: FactoryActionState = { success: false, message: "" };

interface Props {
  factory: { id: number; name: string; isActive: boolean };
}

export default function FactoryRow({ factory }: Props) {
  const t = useTranslations("Factories");
  const tc = useTranslations("Common");
  const [editing, setEditing] = useState(false);
  const [deleting, setDeleting] = useState(false);
  const [error, setError] = useState("");
  const [state, formAction, pending] = useActionState(updateFactoryAction, initState);
  ...
  async function handleDelete() {
    if (!confirm(t("confirmDelete", { name: factory.name }))) return;
    setDeleting(true);
    setError("");
    const res = await deleteFactoryAction(factory.id);
      ...
      <tr className="bg-blue-50/40">
        <td colSpan={3} className="px-5 py-3">
          <form action={formAction} className="flex flex-wrap items-center gap-2">
            ...
            <input name="name" className="input flex-1 min-w-[180px]" defaultValue={factory.name} required />
            <label className="flex items-center gap-1.5 text-sm text-slate-600">
              <input type="checkbox" name="isActive" value="true" defaultChecked={factory.isActive} />
              {t("active")}
            </label>
  ...
  return (
    <tr className="hover:bg-slate-50">
      <td className="px-5 py-3 font-medium text-slate-800">{factory.name}</td>
      <td className="px-4 py-3">
        <span className={clsx(
          "rounded-full px-2.5 py-0.5 text-xs font-semibold",
          factory.isActive ? "bg-green-100 text-green-700" : "bg-slate-100 text-slate-500"
        )}>
          {factory.isActive ? t("active") : t("inactive")}
        </span>
